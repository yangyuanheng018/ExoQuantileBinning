...
data = np.load('./data/sector'+args.sector+'.npz')
...
#sector = pd.read_csv('../tess_tce_csv_new/'+csv_files[int(args.sector)], header=6)

#sector['EB_probability'] = averaged_prediction[:,1]
#sector['PC_probability'] = averaged_prediction[:,2]

df.to_csv('./tce_probability/tce_eb_pc_probability_sector_'+args.sector+'.csv')

print('inferring sector ',args.sector,' complete.')

#sector.to_csv('./tce_probability/tce_eb_pc_probability_sector_'+args.sector+'.csv', index_label='index', columns=['ticid', 'tceid', 'EB_probability', 'PC_probability'])
